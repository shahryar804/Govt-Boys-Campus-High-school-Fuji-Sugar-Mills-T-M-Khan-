# app.py — robust, error-tolerant Flask app
import os
import json
import uuid
...
def read_json(fname, default):
    """
    Read a JSON file and normalize:
    - If file contains a dict with a single list value (e.g. {"students":[...]}) return that list.
    - If file contains a list, return that list.
    - If file is missing or invalid, create with 'default' and return default.
    """
    path = os.path.join(DATA_DIR, fname)
    if not os.path.exists(path):
        # create file with default (prefer list if default is list, otherwise default)
        try:
    ...
    # if file contains dict with a known key, return the inner list
    if isinstance(data, dict):
        # common patterns: {"students": [...]}, {"teachers": [...]}, {"admins": [...]}, {"homework": [...]}
        for key in ("students", "teachers", "admins", "admins", "homework", "submissions", "pending_admissions"):
            if key in data and isinstance(data[key], list):
                return data[key]
        # if dict has one list value, return that list
        list_values = [v for v in data.values() if isinstance(v, list)]
        if len(list_values) == 1:
            return list_values[0]
        # otherwise not a list-like structure; return default
        return default
...
def save_json(fname, obj):
    """
    Save object to JSON file. If obj is a list, save it directly.
    If obj is a dict with a top-level key expected (e.g. {"students": [...]}) you may pass that.
    We'll write the obj as-is if it's dict, otherwise write list.
    """
    path = os.path.join(DATA_DIR, fname)
    try:
...
# initialize defaults (will create files if missing)
...
def login_user(role, user_id):
    session.clear()
    session["role"] = role
    # store id as string for uniformity
...
def get_current_user():
    ...
    if role == "teacher":
        ...
        return next((t for t in teachers if str(t.get("id")) == uid or str(t.get("email")) == uid or str(t.get("username")) == uid or str(t.get("cnic")) == uid), None)
    if role == "student":
        students = read_json("students.json", [])
        return next((s for s in students if str(s.get("id")) == uid or str(s.get("email")) == uid or str(s.get("cnic")) == uid or str(s.get("username")) == uid), None)
    if role == "admin":
        admins = read_json("admin.json", [])
        return next((a for a in admins if str(a.get("id")) == uid or str(a.get("username")) == uid or str(a.get("email")) == uid), None)
    return None
...
def login(role):
    ...
    if request.method == "POST":
        ...
        if role == "student":
            # students.json may be a list or {"students": [...]}
            studs = read_json("students.json", [])
            # iterate safely: ensure each item is a dict
            user = next(
                (s for s in studs if isinstance(s, dict) and
                 (str(s.get("cnic") or "").strip() == identifier or
                  str(s.get("email") or "").strip() == identifier or
                  str(s.get("username") or "").strip() == identifier) and
                 str(s.get("password") or "") == password),
                None
            )
            if user:
                login_user("student", user.get("id") or user.get("email") or user.get("cnic"))
            ...
            user = next(
                (t for t in tlist if isinstance(t, dict) and
                 (str(t.get("username") or "").strip() == identifier or
                  str(t.get("email") or "").strip() == identifier or
                  str(t.get("cnic") or "").strip() == identifier) and
                 str(t.get("password") or "") == password),
                None
            )
            if user:
                login_user("teacher", user.get("id") or user.get("email") or user.get("username"))
            ...
            admins = read_json("admin.json", [])
            user = next(
                (a for a in admins if isinstance(a, dict) and
                 (str(a.get("username") or "").strip() == identifier or
                  str(a.get("email") or "").strip() == identifier) and
                 str(a.get("password") or "") == password),
                None
            )
            if user:
                login_user("admin", user.get("id") or user.get("username") or user.get("email"))
...
def student_dashboard():
    ...
    class_hw = []
    for h in hw_all:
        if str(h.get("class")) == str(s.get("class")):
...
# Admin dashboard & admission decisions
